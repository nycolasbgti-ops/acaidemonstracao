...
export default function AcaiBuilderModal({ product, addons = {}, onClose, onAdd }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-zinc-900 rounded-t-3xl max-h-[92vh] flex flex-col animate-slideUp shadow-xl">
        ...
        <div className="px-5 pt-3 pb-4 border-b border-zinc-800 flex-shrink-0 flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-400 mt-0.5">{fmt(basePrice)}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1">
          ...
          {hasToppings && (
            <>
              ...
              {atLimit && (
                <div className="mx-5 mb-3 px-4 py-3 bg-purple-950/40 rounded-xl border border-purple-800">
                  <p className="text-xs text-purple-300">Limite de {freeToppingsLimit} acompanhamentos atingido.</p>
                </div>
              )}

              {toppings.length === 0 ? (
                <p className="px-5 py-3 text-xs text-gray-500">Nenhum acompanhamento cadastrado.</p>
              ) : (
                <div>
        ...
        {/* ── Rodapé fixo ── */}
        <div className="border-t border-zinc-800 bg-zinc-900 px-5 pt-4 pb-8 flex-shrink-0 space-y-3">

          {(caldaTotal > 0 || extrasTotal > 0) && (
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-xs text-gray-500">{fmt(basePrice)}</span>
              {caldaTotal > 0 && <>
                <span className="text-zinc-600 text-xs">+</span>
                <span className="text-xs text-gray-500">{fmt(caldaTotal)} caldas</span>
              </>}
              {extrasTotal > 0 && <>
                <span className="text-zinc-600 text-xs">+</span>
                <span className="text-xs text-gray-500">{fmt(extrasTotal)} extras</span>
              </>}
              <span className="text-zinc-600 text-xs">=</span>
    ...
    <div className="px-5 pt-6 pb-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className={`text-[11px] font-medium rounded px-2 py-0.5 flex-shrink-0 ${
          badgeActive ? 'bg-purple-900 text-purple-200' : 'bg-zinc-800 text-gray-400'
        }`}>
          {badge}
        </span>
      </div>
...
function RowRadio({ label, sub, isSelected, isLast, onClick }) {
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <span className={`text-[15px] leading-snug ${isSelected ? 'text-white font-medium' : 'text-gray-300'}`}>
          {label}
        </span>
        {sub && <p className="text-[12px] text-gray-500 mt-0.5 truncate">{sub}</p>}
      </div>
      <div className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all ${
        isSelected ? 'border-purple-700' : 'border-zinc-700'
      }`}>
  ...
  return (
    <button
      ...
      className={`w-full flex items-center justify-between px-5 py-4 text-left gap-3
                  transition-colors
                  ${!isLast ? 'border-b border-zinc-800' : ''}
                  ${isDisabled ? 'opacity-30 cursor-not-allowed' : 'hover:bg-zinc-800 active:bg-zinc-700'}`}
    >
      <span className={`flex-1 text-[15px] leading-snug ${isSelected ? 'text-white font-medium' : 'text-gray-300'}`}>
        {label}
      </span>
      <div className="flex items-center gap-2.5 flex-shrink-0">
        {price > 0 && (
          <span className={`text-[13px] ${isSelected ? 'text-purple-400' : 'text-gray-500'}`}>
            + {fmt(price)}
          </span>
        )}
        <div className={`w-5 h-5 rounded-[5px] border-2 flex items-center justify-center transition-all ${
          isSelected ? 'border-purple-700 bg-purple-700' : 'border-zinc-700'
        }`}>
          {isSelected && (
